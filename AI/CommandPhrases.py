# *******************************************
# Alarm Commands
# *******************************************
Generic_Alarm_Request_Commands_List = [
    'can you create an alarm',
    'can you set an alarm',
    'create an alarm',
    'set an alarm',
    'can you create an alarm for me',
    'can you set an alarm for me',
    'create an alarm for me',
    'set an alarm for me'
    ]
Generic_Alarm_Delete_Commands_List = [
    'delete all alarms',
    'remove all alarms',
    'delete all my alarms',
    'remove all my alarms',
    ]
Specific_Alarm_Delete_Commands_List = [
    'delete an alarm',
    'delete a specific alarm',
    ]
Alarm_Disable_Commands_List = [
    'turn off alarm',
    'turn alarm off',
    'disable the alarm',
    ]
global Alarm_Name
Alarm_Name = ""
global Alarm_Time
Alarm_Time = ""
global Alarm_PK
Alarm_PK = ""
# *******************************************
# Reminder Commands
# *******************************************
Generic_Reminder_Request_Commands_List = [
    'can you create a reminder',
    'create a reminder',
    'set a reminder',
    'can you create a reminder for me',
    'can you set a reminder for me',
    'create a reminder for me',
    'set a reminder for me'
    ]
Generic_Reminder_Delete_Commands_List = [
    'delete all reminders',
    'remove all reminders',
    'delete all my reminders',
    'remove all my reminders',
    ]
global Reminder_Name
Reminder_Name = ""

# *******************************************
# Sleep Commands
# *******************************************
Generic_Sleep_Enable_Commands_List = [
    'activate sleep mode',
    'go into sleep mode',
    'goodbye',
    'sleep mode',
    ]
Generic_Sleep_Disable_Commands_List = [
    'hey',
    'hello',
    'good morning',
    'good afternoon',
    'wake up,'
    ]
# *******************************************
# Setup Commands
# *******************************************
Initial_Setup_Commands_List = [
    'start initial setup',
    ]
# *******************************************
# Email Commands
# *******************************************
Generic_Email_Unread_Commands_List = [
    'show me my unread emails',
    'can you show me my unread emails',
    ]
# *******************************************
# Apple Commands
# *******************************************
Generic_Apple_Find_iPhone_Commands_List = [
    'find my iPhone',
    "find my iPhone's location",
    "where is my iPhone"
    ]
